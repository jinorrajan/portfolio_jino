...
class _DesktopViewState extends State<DesktopView> with TickerProviderStateMixin {
      ...
      appBar: AppBar(
        ...
        title:  TextButton(
         child: const Text( "Portfolio",
          style: TextStyle(color: AppColor.textColorIron,fontSize: 20,),),
          onPressed: (){_scrollToSelectedContent(sectionKey1);},
        ),
        actions: [
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            // Homepage
            Homesections(size: size,),

            // About Section
            Aboutsections(size: size,),
  ...
  Widget _buildHoverTextButton(String label, GlobalKey sectionKey,Size size) {
    return MouseRegion(
      onEnter: (_) => hoverController.setHovered(label, true),  // Set hover state for specific button
      onExit: (_) => hoverController.setHovered(label, false),  // Reset hover state for specific button
      child: ValueListenableBuilder(
        valueListenable: hoverController,  // Listen to hover state changes
        builder: (context, _, child) {
          bool isHovered = hoverController.isHovered(label); // Get hover state for this button
            ...
            child: Column(
              ...
              children: [
                ...
                AnimatedContainer(
                  ...
                  transform: Matrix4.translationValues(0, isHovered ? 5 : 0, 0), // Move text down on hover
                  child: Text(
                    label,
                    style: TextStyle(
                      color: isHovered
                      ...
                      fontSize: isHovered ? 16 : 14, // Increase font size when hovered
                    ),
                  ),
                ),
                SizedBox(
